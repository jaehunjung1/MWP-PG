...
def get_response(input_text, num_return_sequences, tokenizer_paraphrase, model_paraphrase, device):
    batch = tokenizer_paraphrase.prepare_seq2seq_batch([input_text], truncation=True, padding='longest', max_length=60,
                                                       return_tensors="pt").to(device)

    translated = model_paraphrase.generate(**batch, max_length=60, num_beams=10,
                                           num_return_sequences=num_return_sequences, temperature=1.5)
    tgt_text = tokenizer_paraphrase.batch_decode(translated, skip_special_tokens=True)
    return tgt_text


def paraphrase(ques_adv_lines, num_map, tokenizer_paraphrase, model_paraphrase, device):
    final_candidates = []
    for j in range(len(ques_adv_lines)):
        num_tokens = []
        for token in ques_adv_lines[j].split(" "):
            if token in num_map:
                num_tokens.append(token)
        paraphrased_texts = get_response(ques_adv_lines[j], 5, tokenizer_paraphrase, model_paraphrase, device)
        if len(num_tokens) != 0:
            candidates = []
            # candidates = [ques_adv_lines[j]]
            for txt in paraphrased_texts:
                temp_num_tokens = []
                for elem in num_tokens:
                    temp_num_tokens.append(elem)
                contain_num_token = len(num_tokens)
                for token in txt.split(" "):
                    if token in temp_num_tokens:
                        contain_num_token -= 1
                        temp_num_tokens.remove(token)
                if contain_num_token == 0:
                    candidates.append(txt)
            if len(candidates) > 0:
                new_text_candidates = candidates
            else:
                new_text_candidates = [ques_adv_lines[j]]
        else:
            # new_text_candidates = [ques_adv_lines[j]]
            # if len(ques_adv_lines[j]) >= 5:
            cands = []
            for txt in paraphrased_texts:
                cands.append(txt)
            # cands.append(ques_adv_lines[j])
            new_text_candidates = cands
        final_candidates.append(new_text_candidates)
    # print(final_candidates)
    all_candidates = list(itertools.product(*final_candidates))
    # print(len(all_candidates))
    if len(all_candidates) > 1000:
        all_candidates = [ques_adv_lines]
    final_texts = []
    for candidate in all_candidates:
        adv_text = []
        for sent in candidate:
            for token in sent.split(" "):
                adv_text.append(token)
        if len(adv_text) <= 500:
            final_texts.append(" ".join(adv_text))
...
def main():
    ...
    if config.full_cv:
        ...
        for z in range(5):
            ...
            pairs_trained, pairs_tested, generate_nums, copy_nums = transfer_num(train_ls, dev_ls,
                                                                                 config.challenge_disp)
            ...
            predict = Prediction(hidden_size=config.hidden_size,
                                 ...
                                 input_size=len(generate_nums), dropout=config.dropout)
            ...
            embedding_optimizer = torch.optim.Adam(embedding.parameters(), lr=config.emb_lr,
            ...
            for epoch in range(config.epochs):
                loss_total = 0
                input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, num_value_batches, graph_batches, group_batches = prepare_train_batch(
                ...
                if config.show_train_acc:
                    ...
                    with torch.no_grad():
                        for train_batch in train_pairs:
                            batch_graph = get_single_example_graph(train_batch[0], train_batch[1], train_batch[7],
                                                                   train_batch[4], train_batch[5])
                            train_res = evaluate_tree(config, train_batch[0], train_batch[1], generate_num_ids,
                                                      embedding, encoder, predict, generate,
                                                      merge, input_lang, output_lang, train_batch[4], train_batch[5],
                                                      batch_graph, test_batch[7], beam_size=config.beam_size)
                            train_val_ac, train_equ_ac, _, _ = compute_prefix_tree_result(train_res, train_batch[2],
                                                                                          output_lang, train_batch[4],
                                                                                          train_batch[6])
                ...
                for test_batch in test_pairs:
                    ...
                    with open(config.outputs_path + '/outputs.txt', 'a') as f_out:
                        ...
                        f_out.write(
                            'Source: ' + stack_to_string(sentence_from_indexes(input_lang, test_batch[0])) + '\n')
                        f_out.write(
                            'Target: ' + stack_to_string(sentence_from_indexes(output_lang, test_batch[2])) + '\n')
                        f_out.write(
                            'Generated: ' + stack_to_string(sentence_from_indexes(output_lang, test_res)) + '\n')
                        if config.nums_disp:
        ...
        if is_train:
            ...
            model_paraphrase = PegasusForConditionalGeneration.from_pretrained(model_name_paraphrase).to(device)
            ...
            for test_batch in tqdm(test_pairs):
                num_map = {}
                for i in range(len(pairs_tested[cur_idx][2])):
                    cur_num = pairs_tested[cur_idx][2][i]
                    num_map["NUM" + str(i)] = cur_num

                temp_text = " ".join(pairs_tested[cur_idx][0])
                temp_text_adv = []
                idx = 0
                for token in temp_text.split(" "):
                    if token == "NUM":
                        temp_text_adv.append("NUM" + str(idx))
                        idx += 1
                    else:
                        temp_text_adv.append(token)
                adv_text = " ".join(temp_text_adv)
                # cur_idx+=1
                ques_adv_lines = re.split(' , | . ', adv_text)
                ques_text = ques_adv_lines[-1]
                ques_adv_lines = ques_adv_lines[0:len(ques_adv_lines)]
                result = paraphrase(ques_adv_lines, num_map, tokenizer_paraphrase, model_paraphrase, device)
                flag = 0
                orig_test_batch = test_batch
                cntr = 0
                can_adv = 0
                for txt in result:
                    new_text = txt.split(" ")
                    ...
                    num_lst = []
                    final_adv_text = []
                    for idxx in range(len(new_text)):
                        if new_text[idxx] in num_map:
                            new_idx.append(idxx)
                            num_lst.append(num_map[new_text[idxx]])
                            final_adv_text.append("NUM")
                        else:
                            final_adv_text.append(new_text[idxx])
                    new_adv_pair = []
                    temp_lst = []
                    for num in new_idx:
                        temp_lst.append(num - 1)
                        temp_lst.append(num)
                        temp_lst.append(num + 1)
                    last_idx = len(final_adv_text) - 1
                    temp_lst.append(last_idx)
                    temp_lst.append(last_idx - 1)
                    temp_lst.append(last_idx - 2)

                    new_adv_pair.append((final_adv_text, pairs_tested[cur_idx][1], num_lst, new_idx, temp_lst))

                    try:
                        input_lang, output_lang, train_pairs, final_adv_pair = prepare_data(config, logger,
                                                                                            pairs_trained, new_adv_pair,
                                                                                            config.trim_threshold,
                                                                                            generate_nums, copy_nums,
                                                                                            input_lang, output_lang,
                                                                                            tree=True)
                    except:
                        continue

                    test_batch = final_adv_pair[0]
                    try:
                        batch_graph = get_single_example_graph(test_batch[0], test_batch[1], test_batch[7],
                                                               test_batch[4], test_batch[5])
                    except:
                        continue
                    test_res = evaluate_tree(config, test_batch[0], test_batch[1], generate_num_ids, embedding, encoder,
                                             predict, generate,
                    ...
                    if cur_result == 0:
                        flag = 1
                        final_adv_batch = final_adv_pair[0]
                        break
                    cntr += 1
                eval_total += 1
                if flag == 0:
                    test_batch = orig_test_batch
                else:
                    test_batch = final_adv_batch
                ...
                with open(config.outputs_path + '/outputs_test.txt', 'a') as f_out:
                    f_out.write('Example: ' + str(ex_num) + '\n')
