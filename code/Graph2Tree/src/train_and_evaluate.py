...
def train_tree(config, input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch,
               num_value_batch, group_batch, generate_nums,
               embedding, encoder, predict, generate, merge, embedding_optimizer, encoder_optimizer, predict_optimizer,
               generate_optimizer,
    ...
    if config.embedding == 'bert' or config.embedding == 'roberta':
        ...
        for bat in range(len(group_batch)):
            try:
                new_group_batch.append(
                    [index_retrieve[bat][index1] for index1 in group_batch[bat] if index1 < len(index_retrieve[bat])])
            except:
                pdb.set_trace()
...
def evaluate_tree(config, input_batch, input_length, generate_nums, embedding, encoder, predict, generate, merge,
                  input_lang, output_lang, num_value, num_pos, batch_graph, group_example, beam_size=5, english=False,
    ...
    if config.embedding == 'bert' or config.embedding == 'roberta':
        ...
        try:
            new_group_example = [index_retrieve[0][index1] for index1 in group_example if
                                 index1 < len(index_retrieve[0])]
        except:
            pdb.set_trace()

        batch_graph = get_single_example_graph(token_ids.cpu().tolist()[0], input_len1[0], new_group_example, num_value,
                                               num_pos)
        batch_graph = torch.LongTensor(batch_graph)
