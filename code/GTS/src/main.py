	...
	if config.full_cv:
		...
		if is_train:
			...
			for test_batch in test_pairs:
				...
				ques_adv_lines = ques_adv_lines[0:len(ques_adv_lines)-1]
				new_text = []
				for token in ques_text.split(" "):
					new_text.append(token)

				new_text.append("given")
				new_text.append("that")
				for sent in ques_adv_lines:
				...
				new_adv_pair = []
				new_adv_pair.append((final_adv_text, pairs_tested[cur_idx][1], num_lst, new_idx))
				input_lang, output_lang, train_pairs, final_adv_pair = prepare_data(config, logger, pairs_trained, new_adv_pair, config.trim_threshold, generate_nums, copy_nums, input_lang, output_lang, tree=True)
				...
				with open(config.outputs_path + '/outputs_test.txt', 'a') as f_out:
					...
					if config.nums_disp:
						...
						f_out.write('Numbers in Predicted Equation: ' + str(pred_nums) + '\n')
					f_out.write('Result: ' + str(cur_result) + '\n' + '\n')
					f_out.write('Number List '+str(pairs_tested[cur_idx][2]))
					f_out.close()
				cur_idx += 1
				ex_num += 1
